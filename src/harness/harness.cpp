...
ValidationResult Validator::validate_against_reference(
    MatmulImplementation &candidate, MatmulImplementation &reference,
    std::size_t size, float tolerance, unsigned int seed) const {
  Matrix a(size);
  Matrix b(size);
...
BenchmarkResult BenchmarkRunner::run(MatmulImplementation &implementation,
                                     MatmulImplementation &reference) const {
  Validator validator;
  const ValidationResult validation = validator.validate_against_reference(
      implementation, reference, config_.validation_size, config_.tolerance,
      config_.seed);
  ...
  result.min_ms =
      *std::min_element(result.run_times_ms.begin(), result.run_times_ms.end());
  result.max_ms =
      *std::max_element(result.run_times_ms.begin(), result.run_times_ms.end());
  result.mean_ms = std::accumulate(result.run_times_ms.begin(),
                                   result.run_times_ms.end(), 0.0) /
