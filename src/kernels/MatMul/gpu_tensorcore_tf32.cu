...
__global__ void matmul_tensorcore_tf32_kernel(const float *a, const float *b,
                                              float *c, int n) {
  ...
  __align__(32) __shared__ float a_tile[WMMA_M * WMMA_K];
  __align__(32) __shared__ float b_tile[WMMA_K * WMMA_N];
  ...
  for (int k0 = 0; k0 < n; k0 += WMMA_K) {
    for (int idx = lane; idx < WMMA_M * WMMA_K; idx += WARP_SIZE) {
      ...
      const float value = (global_row < n && global_col < n)
                              ...
                              : 0.0f;
      a_tile[idx] = nvcuda::wmma::__float_to_tf32(value);
    }
    ...
    for (int idx = lane; idx < WMMA_K * WMMA_N; idx += WARP_SIZE) {
      ...
      const float value = (global_row < n && global_col < n)
                              ...
                              : 0.0f;
      b_tile[idx] = nvcuda::wmma::__float_to_tf32(value);
    }
...
class GpuTensorcoreTf32Matmul final : public MatmulImplementation {
public:
  const char *name() const override { return "gpu_tensorcore_tf32_cuda"; }
  ...
  void multiply(const Matrix &a, const Matrix &b, Matrix &c) override {
    ...
    if (prop.major < 8) {
      throw std::runtime_error(
          "TF32 Tensor Cores require compute capability 8.0+");
    }
    ...
    matmul_tensorcore_tf32_kernel<<<grid, block>>>(d_a.get(), d_b.get(),
                                                   d_c.get(), n);
    checkCuda(cudaGetLastError(), "launch matmul_tensorcore_tf32_kernel");
    checkCuda(cudaDeviceSynchronize(), "sync matmul_tensorcore_tf32_kernel");
...
std::unique_ptr<MatmulImplementation> make_gpu_tensorcore_tf32() {
  return std::make_unique<GpuTensorcoreTf32Matmul>();
}
