...
__global__ void matmul_tiled_kernel(const float* a, const float* b, float* c, int n) {
    ...
    for (int tile_idx = 0; tile_idx < tile_count; ++tile_idx) {
        ...
        // Compute a tile sized dot product for 1 set of tiles, using shared memory for reads
        for (int k = 0; k < TILE; ++k) {
            acc += a_tile[threadIdx.y][k] * b_tile[k][threadIdx.x];
        }

        __syncthreads(); // Prevents threads from writing the next tile before others finish reading
    }
